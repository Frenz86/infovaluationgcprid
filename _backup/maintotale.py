...
selection_multipli = ["EnterpriseValue_ratio_Sales","EnterpriseValue_ratio_Ebitda","EnterpriseValue_ratio_Ebit","Price_ratio_Earning",
...
async def get_all_multipli_year_sectorear(requests:Request,user:str="EARnext",anno:int= 2020,settore:str="01",
...
async def get_all_sector_year_avgear(requests:Request,user:str="EARnext",anno:int= 2021,settore: str="01",
                                    current_user: User = Depends(get_current_active_user)):
    statement = select(Medie2)
    obj = session.exec(statement).all()
    df = sqlmodel_to_df(obj)
...
async def get_all_branches_years_sectorear(requests:Request,user:str="EARnext",anno:int= 2021,settore:str="01",
                                            current_user: User = Depends(get_current_active_user)):
    statement = select(Fasce2)
    obj = session.exec(statement).all()
    df = sqlmodel_to_df(obj)
