...
@app.get('/list_sectors2',status_code=status.HTTP_200_OK,)
...
def append_googlesheet(endpoint:str,sector:str,user:str):
    ...
    tz = pytz.timezone('CET')#central europe times
    now = datetime.now(tz)
    # dd/mm/YY H:M:S
...
######################################################################################
########################### MULTIPLI # EARNEXT #######################################
selection_multipli_rid = ["Descrizione_settore","EnterpriseValue_ratio_Ebitda","EnterpriseValue_ratio_Ebit","Beta_activity"]

def multipli_year_sector_rid(df,year,sector):
    d= json.loads(df.loc[(df['anno'] == year) & (df['settore'] ==sector)].groupby(['anno','settore'])[selection_multipli_rid] \
...
@app.get('/EARMULTIPLIridsecond',tags=["Multipli - EAR"],)
async def get_all_multipli_rid_sectorsecond(requests:Request,user:str="EARnext",anno:int= 2021,settore:str="01",
                                            current_user: User = Depends(get_current_active_user)):
    statement = select(Multipli2)
    obj = session.exec(statement).all()
    df = sqlmodel_to_df(obj)
    result = multipli_year_sector_rid(df,anno,settore)
...
@app.get('/EARMULTIPLIridfourth',tags=["Multipli - EAR"],)
async def get_all_multipli_rid_four(requests:Request,user:str="EARnext",anno:int= 2021,settore:str="0111",
                                            current_user: User = Depends(get_current_active_user)):
...
# @app.get('/settori2cifre/{settore_id}', response_model=Multipli2,tags=["Multipli2"])
# async def get_a_sector_byidsecond(settore_id:int):
...
# ## GET a sector by ID (only first one)
# @app.get('/settori4cifre/{settore_id}', response_model=Multipli4,tags=["Multipli4"])
# async def get_a_sector_byidsecond(settore_id:int):
#     statement = select(Multipli4).where(Multipli4.id==settore_id)
#     result = session.exec(statement).first()
#     if result == None:
#         raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
#     return result
